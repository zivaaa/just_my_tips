...
Небольшое приложене с гугл картой и геолокацией.

Это простая активити которая отображает маркер на карте и выводит текущие координаты.

Пусть весб код обертки над locationManager тут будет, а то лень искать его в коде потом если потребуется.
